...
'''
Test script
*WARNING* Don't run this on a production privcy server! *WARNING*
...
import privcyrpc
from privcyrpc.exceptions import privcyException, InsufficientFunds
...
if __name__ == "__main__":
    ...
    if args.config:
        from privcyrpc.config import read_config_file
        cfg = read_config_file(args.config)
    else:
        from privcyrpc.config import read_default_config
        cfg = read_default_config(None)
    port = int(cfg.get('rpcport', '120103' if cfg.get('testnet') else '20103'))
    rpcuser = cfg.get('rpcuser', '')

    connections = []
    if not args.nolocal:
        local_conn = privcyrpc.connect_to_local()  # will use read_default_config
        connections.append(local_conn)
    if not args.noremote:
        remote_conn = privcyrpc.connect_to_remote(
    ...
    for conn in connections:
        ...
        privcyaddress = conn.getnewaddress(account)
        conn.setaccount(privcyaddress, account)
        ...
        #conn.sendtoaddress(privcyaddress, amount, comment=None, comment_to=None)
        conn.getreceivedbyaddress(privcyaddress)
        ...
        for message in messages:
            signature = conn.signmessage(privcyaddress, message)
            assert(conn.verifymessage(privcyaddress, signature, message) is True)
        ...
        try:
            conn.sendtoaddress(privcyaddress, 10000000000)
